...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  void initState() {
    ...
    _controller.text = '';
    // 注册全局键盘监听
    RawKeyboard.instance.addListener(_handleRawKeyboardEvent);
    // 在页面加载后让金额输入框获得焦点
  ...
  @override
  void dispose() {
    RawKeyboard.instance.removeListener(_handleRawKeyboardEvent);
    _controller.dispose();
  ...
  void _handleRawKeyboardEvent(RawKeyEvent event) {
    ...
    // 空格键：仅在 Received Amount TextField 有焦点时才处理
    if (key == LogicalKeyboardKey.space) {
      if (_amountInputFocusNode.hasFocus) {
        setState(() {
          isCash = false;
            ...
            if (countdownSeconds > 0) {
              Future.delayed(Duration(seconds: 1), () {
                if (context.mounted) {
                  ...
                  if (countdownSeconds == 0) {
                    // 倒计时结束，自动执行 Continue 逻辑
                    Navigator.of(dialogContext).pop(); // 关闭对话框
                    Navigator.of(context).pop(true); // 返回 true 给 OrderPage
                  }
                }
              });
            }
  ...
  /// Show missing amount dialog (English message)
  void _showMissingAmountDialog() {
    showDialog(
      context: context,

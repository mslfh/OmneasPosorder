...
class OrderMatchResult {
  ...
  List<String> getMismatchedItems() {
    ...
    if (onlineLastOrderMatch != null) items.add('Online最新号: $onlineLastOrderMatch');
    if (terminalCountMatch != null) items.add('Terminal订单数: $terminalCountMatch');
    if (terminalSyncedMatch != null) items.add('Terminal已同步数: $terminalSyncedMatch');
    if (terminalLastOrderMatch != null) items.add('Terminal最新号: $terminalLastOrderMatch');
...
class OrderMatchService {
  ...
  Future<OrderMatchResult> verifyOrdersMatch() async {
    try {
      ...
      final terminalOrders = localOrders.where((o) => !o.isOnlineOrder).toList();

      // 已同步的online订单
      final onlineSyncedOrders = onlineOrders
       ...
       final latestRemoteOrderId = await _databaseService.getMaxRemoteOrderId() ?? 0;
      ...
      if (onlineSyncedOrders.length != data.onlineSyncedCount) {
        onlineSyncedError = '本地${onlineSyncedOrders.length} != 服务器${data.onlineSyncedCount}';
      }

      // 验证online最新order_no
      final onlineLastOrderNo = _getLastOrderNo(onlineOrders);
      if (onlineLastOrderNo != data.onlineLastOrderNo) {
        onlineLastOrderError = '本地$onlineLastOrderNo != 服务器${data.onlineLastOrderNo}';
      }

      // 验证terminal订单数量
      if (terminalOrders.length != data.terminalCount) {
        terminalCountError = '本地${terminalOrders.length} != 服务器${data.terminalCount}';
      }

      // 验证terminal已同步数量
      if (terminalSyncedOrders.length != data.terminalSyncedCount) {
        terminalSyncedError = '本地${terminalSyncedOrders.length} != 服务器${data.terminalSyncedCount}';
      }
      ...
      if (terminalLastOrderNo != data.terminalLastOrderNo) {
       ...
       if (latestRemoteOrderId != serverLatestOrderId) {
         latestOrderIdError = '本地$latestRemoteOrderId != 服务器$serverLatestOrderId';
       }
